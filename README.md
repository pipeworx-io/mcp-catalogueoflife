...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/catalogueoflife_search \
  -H 'Content-Type: application/json' \
  -d '{"query":"Panthera leo"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/catalogueoflife_search`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "catalogueoflife": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-catalogueoflife"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-catalogueoflife
```
